...
function getBirthdayState(user: UserDoc, today = getTodayParts()) {
...
async function syncBirthdayWishForUser(uid: string, user: UserDoc, today = getTodayParts()) {
...
async function syncAutomaticBirthdayWishes() {
  ...
  for (const doc of snap.docs) {
    const changed = await syncBirthdayWishForUser(doc.id, doc.data() as UserDoc, today);
